...
#define POINTS 4
#define TEXT 5
#define RUBBERBAND_LINE 6 // RubberBand를 이용한 LINE 추가
#define RUBBERBAND_RECT 7 // RubberBand를 이용한 RECTANGLE 추가
...
void mouse(int btn, int state, int x, int y)
{
    ...
    if (btn == GLUT_LEFT_BUTTON && draw_mode == 0 && state == GLUT_UP) // RUBBERBAND_LINE
    {
    ...
    else if (btn == GLUT_LEFT_BUTTON && draw_mode == RUBBERBAND_LINE)
    {
        if (count == 0)
        {
            count++;
    ...
    else if (btn == GLUT_LEFT_BUTTON && draw_mode == RUBBERBAND_RECT) // RUBBERBAND_RECT
    {
        if (count == 0)
        {
            count++;
    ...
    else if (btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN && draw_mode != 0)
    {
        glPushAttrib(GL_ALL_ATTRIB_BITS);
...
int pick(int x, int y)
{
    ...
    else if (x < 3 * ww / 5) return RUBBERBAND_LINE;
    else if (x < 7 * ww / 10) return RUBBERBAND_RECT;
    else return 0;
}
...
void pixel_menu(int id)
{
    ...
    else if (id == 3) glLineWidth(size = 2 * size);
    else if (id == 4) glLineWidth(size = size / 2);
}
...
void display(void)
{
    ...
    screen_box(0, wh - ww / 10, ww / 10); // 라인
    glColor3f(1.0, 0.0, 0.0);
    screen_box(ww / 10, wh - ww / 10, ww / 10); // 사각형
    glColor3f(0.0, 1.0, 0.0);
    screen_box(ww / 5, wh - ww / 10, ww / 10); // 삼각형
    glColor3f(0.0, 0.0, 1.0);
    screen_box(3 * ww / 10, wh - ww / 10, ww / 10); // 점
    glColor3f(1.0, 1.0, 0.0);
    screen_box(2 * ww / 5, wh - ww / 10, ww / 10); // 텍스트

    glColor3f(1.0, 0.0, 1.0); 
    screen_box(ww / 2, wh - ww / 10, ww / 10); // 러버밴드 라인
    glColor3f(0.0, 1.0, 1.0); 
    screen_box(3 * ww / 5, wh - ww / 10, ww / 10); // 러버밴드 사각형
    glColor3f(1.0, 1.0, 1.0);
    screen_box(ww / 2 + ww / 40, wh - ww / 10 + ww / 40, ww / 20); // 메뉴 6 라인

    glLogicOp(GL_XOR);
    glColor3f(0.0, 0.0, 0.0);

    glBegin(GL_LINES);
    glVertex2i(ww / 2 + wh / 85, wh - ww / 20);
    glVertex2i(ww / 2 + wh / 40 + ww / 15, wh - ww / 20);
    glEnd();

    glLogicOp(GL_COPY); 
    glColor3f(0.0, 0.0, 0.0);

    glBegin(GL_LINE_LOOP); // 메뉴 7 사각형 1
    glVertex2i(3 * ww / 5 + ww / 40, wh - ww / 20);
    glVertex2i(3 * ww / 5 + ww / 20, wh - ww / 20);
    glVertex2i(3 * ww / 5 + ww / 20, wh - ww / 40);
    glVertex2i(3 * ww / 5 + ww / 40, wh - ww / 40);
    glEnd();
    glBegin(GL_LINE_LOOP); // 메뉴 7 사각형 2
    glVertex2i(3 * ww / 5 + ww / 40, wh - ww / 40 - ww / 20);
    glVertex2i(3 * ww / 5 + ww / 40 + ww / 20, wh - ww / 40 - ww / 20);
    glVertex2i(3 * ww / 5 + ww / 40 + ww / 20, wh - ww / 40);
    glVertex2i(3 * ww / 5 + ww / 40, wh - ww / 40);
    glEnd();
...
void glutMotionFun(void(*func));
void motion(int x, int y)
{
    y = wh - y;
    if (draw_mode == RUBBERBAND_LINE)
    {
    ...
    else if (draw_mode == RUBBERBAND_RECT)
    {
        glEnable(GL_COLOR_LOGIC_OP);
